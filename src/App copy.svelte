...
// ------------I/O
	...
	const loadSimMatrix = async () => {
  		try {
			const csvFilePath = 'distances/wasserstein_C_l2.csv';
...
// ------------LOCAL STORAGE
	...
	const loadLabels = () => {
		...
		// generatedPaths = [];
		// console.log(JSON.parse(genPathsJSON))
		// generatedPaths = JSON.parse(genPathsJSON) ? genPathsJSON : [];
		
		// const fakePathsJSON = localStorage.getItem('fakePaths');
		// fakePaths = JSON.parse(fakePathsJSON) ? fakePathsJSON : [];
	}
// ------------TAGGING
	let showTag = false;
	let selectedTag = '';
